...
window.addEventListener("pageshow", () => {
  ...
  function initializePincodeCheck() {
    ...
    console.log("[v0] Saved pincode:", savedPincode)
    ...
    if (savedPincode && AVAILABLE_PINCODES.includes(savedPincode)) {
      ...
      // Show modal for all users on first visit or if pincode invalid
      console.log("[v0] Opening pincode modal for user")
      openPincodeModal()
    }
  }
...
function confirmPincode() {
  ...
  if (pincode && pincode.length === 6 && AVAILABLE_PINCODES.includes(pincode)) {
    ...
    closePincodeModal()   // ✅ THIS WILL NOW WORK
