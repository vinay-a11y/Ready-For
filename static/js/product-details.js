...
function renderProductDetails(product) {
  ...
  selectedVariant = variants.length
    ? { ...variants[0] }
    : { packing: "250gm", price: 0 }

  let discountPercentage = 0
  if (product.original_price && selectedVariant.price) {
    discountPercentage = Math.round(
      ((product.original_price - selectedVariant.price) / product.original_price) * 100
    )
  }
  ...
  container.innerHTML = `
    <div class="product-layout">
      ...
      <!-- LEFT: IMAGE + DESKTOP DESCRIPTION -->
      <div class="product-gallery">
        <div class="product-image">
          <img src="${product.image_url || "/placeholder.svg?height=400&width=400"}"
               alt="${product.item_name}"
               loading="lazy"
               onerror="this.src='/placeholder.svg?height=400&width=400'" />
        </div>
          ...
          <div class="product-tabs">
            ...
            <div class="tab-content" id="specifications">
              <ul>
                ...
                <li><strong>Weight:</strong> ${selectedVariant.packing}</li>
      ...
      <div class="product-info">
        ...
        <!-- MOBILE ONLY: DESCRIPTION AFTER ADD TO CART -->
        <div class="product-details-section mobile-only">
          <h3>Description</h3>
          <p>${product.description || "No description available."}</p>
...
async function loadRelatedProducts() {
  try {
    ...
    const processedProducts = products
      .filter(p => p.id && p.item_name && Array.isArray(p.variants) && p.variants.length)
      .map(p => ({
        id: p.id,
        ...
        variants: [...p.variants].sort((a, b) => a.price - b.price),
      }))
    ...
    renderRelatedProducts(relatedProducts.slice(0, 8))

  } catch (error) {
    console.error("Related products error:", error)
    document.getElementById("relatedGrid").innerHTML =
      `<p style="text-align:center">Failed to load related products</p>`
  }
}
    ...
    return `
      <div class="product-card" onclick="goToProduct(${product.id})">
        ...
        <div class="product-info">
          ...
          <div class="product-price" id="relatedPrice-${product.id}">
        ...
        : ""
          ...
          <button
            class="add-to-cart"
            onclick="event.stopPropagation(); addToCartFromRelated(${product.id})"
          >
